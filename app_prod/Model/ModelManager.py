...
class ModelManager(ABC_ModelManager):
    ...
    @classmethod 
    def get_failure_label(cls, classification_model: ABC_Model, inference_data:pd.DataFrame) -> int:
        failure_label = classification_model.predict(inference_data)
        return failure_label 

    @classmethod 
    def get_well_features(cls, feature_extractor:ABC_Feature_Extractor, inference_data:pd.DataFrame) -> tuple:
    ...
    @classmethod 
    def get_days_to_load_off(cls, regression_model:ABC_Model, inference_data:pd.DataFrame) -> int:
    ...
    def get_all_notifications(self, inference_data: pd.DataFrame)->dict:
        """Generate a csv with all the notifications for the current day assessment for a given well inference data 

        Args:
            inference_data (pd.DataFrame): latest inference data batch for one well  

        Returns:
            dict: ROC model
        """
        response = {'body':{}, 
                    'model_version': self.get_model_version({"classification_model":self.classification_model, "regression_model":self.regression_model})}
        ...
        try:
            ...
            response['status'] = 0
            response['body'] = {"TREND_DATA":trend_date,
                    "WELL_CD":well_cd,
    ...
    def get_inference_output(self, inference_data_dict:dict[str,pd.DataFrame])->dict:
        response = {} 
        for well in inference_data_dict: 
            response[well] = self.get_all_notifications(inference_data_dict[well])
        return response 
